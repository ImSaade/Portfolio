...
function ProjectCard({ title, description, imgSrc, navId, codeLink, demoLink, technologies, className }) {
  ...
  return (
    <>
      ...
      <div className={`bg-[#f9f9f9] rounded-lg shadow-md mb-8 pt-3 max-w-full ${className}`}>
        <div className={`flex flex-col lg:flex-row ${className}`}>
          {/* Image Section */}
          <div className="lg:w-7/12 w-full">
            <img src={imgSrc} alt="Project Preview" onClick={handleShow} className="rounded mt-2 w-full cursor-pointer" loading="lazy" />
          </div>
      ...
      {/* Lightbox Modal */}
      {showLightbox && (
        <div className="fixed inset-0 bg-black bg-opacity-80 flex items-center justify-center z-50" onClick={handleClose} role="dialog" aria-modal="true">
          <div className="relative max-w-[90%]">
            <img src={imgSrc} alt="Project Full View" className="max-h-[90vh] w-auto rounded shadow-lg" loading="lazy" />
            <button onClick={handleClose} aria-label="Close" className="absolute top-3 right-3 bg-white bg-opacity-20 text-white p-2 rounded-full hover:bg-opacity-40">
              ✕
            </button>
          </div>
        </div>
